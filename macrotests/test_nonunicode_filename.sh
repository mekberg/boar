...
$PYTHON_BINARY -c "open(b'Tree/illegal\xa0\xa1', 'w')" || exit 1
...
grep -i "Found a filename that is illegal under the current file system encoding (UTF-8):" output.txt || { echo "Unexpected output:"; cat output.txt; exit 1; }

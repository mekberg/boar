...
index 0000000000000000000000000000000000000000..63619b8b42bf3a3bedc2d02c807f30586853434a
--- /dev/null
+++ b/AGENTS.md
@@ -0,0 +1,43 @@
...
+- **`evt/`** – External verification tools/scripts for manifests, often used in regression testing.
+- **`run_tests.sh` / `run_tests_nodedup.sh`** – Convenience scripts that create a temporary cache, select the Python interpreter from the `boar` shebang, and run unit tests followed by macro tests either with or without the deduplication extension present.
+
+## Conventions and tips
+
+- Many modules import `from __future__` and `future`/`past` helpers to maintain compatibility with both Python 2 and 3. Preserve those imports when creating new modules and prefer the compatibility helpers (`builtins`, `past.builtins`, etc.) when dealing with strings or iteration.
